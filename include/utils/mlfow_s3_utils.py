...
class MLflowS3Manager:
    ...
    def upload_artifact_to_s3(self, local_path: str, run_id: str, artifact_path: Optional[str] = None):
        ...
        try:
            # Construct S3 key
            if artifact_path:
                s3_key = f"{run_id}/artifacts/{artifact_path}/{os.path.basename(local_path)}"
            else:
                s3_key = f"{run_id}/artifacts/{os.path.basename(local_path)}"
        ...
        try:
            ...
            for root, dirs, files in os.walk(artifacts_dir):
                for file in files:
                    ...
                    # Upload to S3
                    s3_key = f"{run_id}/artifacts/{relative_path}"
