...
class ModelTrainer:
    def __init__(self, config_path: str = "/usr/local/airflow/include/config/model_config.yml"):
    ...
    def prepare_data(self, df: pd.DataFrame, target_col: str = 'total_revenue',
    ...
    def train_all_models(self, train_df: pd.DataFrame, val_df: pd.DataFrame,
                        test_df: pd.DataFrame, target_col: str = 'total_revenue',
        ...
        try:
            ...
            self.mlflow_manager.log_params({f"lgb_top_feature_{i}": f"{row['feature']} ({row['importance']:.4f})" 
                                           for i, (_, row) in enumerate(lgb_importance.iterrows())})
