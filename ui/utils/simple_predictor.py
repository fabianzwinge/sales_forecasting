    ...
    def prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'total_revenue' in df.columns:
            ...
            for lag in [1, 2, 3, 7, 14, 21, 30]:
                df[f'total_revenue_lag_{lag}'] = df['total_revenue'].shift(lag)
            
            # Rolling statistics for different windows
            for window in [3, 7, 14, 21, 30]:
                df[f'total_revenue_rolling_{window}_mean'] = df['total_revenue'].rolling(window).mean()
                df[f'total_revenue_rolling_{window}_std'] = df['total_revenue'].rolling(window).std()
                df[f'total_revenue_rolling_{window}_min'] = df['total_revenue'].rolling(window).min()
                df[f'total_revenue_rolling_{window}_max'] = df['total_revenue'].rolling(window).max()
                df[f'total_revenue_rolling_{window}_median'] = df['total_revenue'].rolling(window).median()
            
            # Fill NaN values with appropriate defaults
            sales_mean = df['total_revenue'].mean()
            for col in df.columns:
                if 'total_revenue_lag' in col or 'total_revenue_rolling' in col:
                    if 'std' in col:
